...
func getRoots(_ context.Context, li *logInfo, w http.ResponseWriter, _ *http.Request) (int, error) {
	...
	rawCerts := make([][]byte, 0, len(li.log.CertValidationOpts.trustedRoots.RawCertificates()))
	for _, cert := range li.log.CertValidationOpts.trustedRoots.RawCertificates() {
...
func verifyAddChain(li *logInfo, req ct.AddChainRequest, expectingPrecert bool) ([]*x509.Certificate, error) {
	...
	validPath, err := validateChain(req.Chain, li.log.CertValidationOpts)
	if err != nil {
