...
**Prevention:** Use `http.MaxBytesHandler` at the router level (`mux.Handle`) to enforce a global limit (4MB) on all relevant endpoints. Ensure underlying handlers catch `*http.MaxBytesError` from `io.ReadAll` to explicitly return a 413 status code.
