...
func TestAddChainLargeBody(t *testing.T) {
	...
	largeBody := `{"chain": []}` + strings.Repeat(" ", 5*1024*1024)

	resp, err := http.Post(server.URL+rfc6962.AddChainPath, "application/json", bytes.NewBufferString(largeBody))
	if err != nil {
		t.Fatalf("http.Post failed: %v", err)
	}
	defer resp.Body.Close()

	// Since we now limit the body size to 4MB, and we sent 5MB, we expect 413.
