...
func TestValidate(t *testing.T) {
	...
	if fakeCARoots.AppendCertsFromPEMs([]byte(testdata.FakeCACertPEM)) <= 0 {
		t.Fatal("failed to load fake root")
	}
	if fakeCARoots.AppendCertsFromPEMs([]byte(testdata.FakeRootCACertPEM)) <= 0 {
		t.Fatal("failed to load fake root")
	}
	if fakeCARoots.AppendCertsFromPEMs([]byte(testdata.CACertPEM)) <= 0 {
		t.Fatal("failed to load CA root")
	}
	if fakeCARoots.AppendCertsFromPEMs([]byte(testdata.RealPrecertIntermediatePEM)) <= 0 {
...
func TestPreIssuedCert(t *testing.T) {
	...
	if roots.AppendCertsFromPEMs([]byte(testdata.CACertPEM)) <= 0 {
