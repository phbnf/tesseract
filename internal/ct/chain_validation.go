...
type chainValidator struct {
	...
	// extKeyUsagesMap contains the list of EKUs to use during chain verification (map for O(1) lookup).
	extKeyUsagesMap map[x509.ExtKeyUsage]bool
	// rejectExtIdsMap contains a list of X.509 extension IDs to reject during chain verification (map for O(1) lookup).
...
func (cv *chainValidator) refreshMaps() {
	if len(cv.extKeyUsages) > 0 {
		cv.extKeyUsagesMap = make(map[x509.ExtKeyUsage]bool, len(cv.extKeyUsages))
		for _, eku := range cv.extKeyUsages {
			cv.extKeyUsagesMap[eku] = true
		}
	} else {
		cv.extKeyUsagesMap = nil
	}

	if len(cv.rejectExtIds) > 0 {
		cv.rejectExtIdsMap = make(map[string]bool, len(cv.rejectExtIds))
		for _, id := range cv.rejectExtIds {
			cv.rejectExtIdsMap[id.String()] = true
		}
	} else {
		cv.rejectExtIdsMap = nil
	}
}
