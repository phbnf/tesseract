...
type chainValidator struct {
	...
	// extKeyUsagesMap is a set of EKUs to use during chain verification.
	...
	// rejectExtIdsMap is a set of X.509 extension IDs to reject during chain verification.
...
func (cv chainValidator) validate(chain []*x509.Certificate) ([]*x509.Certificate, error) {
	...
	if len(cv.extKeyUsagesMap) > 0 {
		...
		for _, certEKU := range cert.ExtKeyUsage {
			if _, ok := cv.extKeyUsagesMap[certEKU]; ok {
				good = true
				break
			}
		}
