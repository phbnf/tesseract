...
func TestBuildV1MerkleTreeLeafForCert(t *testing.T) {
	...
	SignSCT, err := setupSCTSigner(fakeSignature)
	if err != nil {
...
func setupSCTSigner(fakeSig []byte) (signSCT, error) {
	block, _ := pem.Decode([]byte(testdata.DemoPublicKey))
	...
	return NewSCCTSignFunc(testdata.NewSignerWithFixedSig(key, fakeSig)), nil
}
