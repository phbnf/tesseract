...
var (
	...
	projectID          = flag.String("project_id", "", "AWS ProjectID.")
...
func main() {
	...
	// TODO(phboneff): move to something else
	signer, err := pem.ReadPrivateKeyFile(*privKey, *privKeyPass)
	if err != nil {
		klog.Exitf("Can't open key: %v", err)
	}

	vCfg, err := sctfe.ValidateLogConfig(*origin, *projectID, *bucket, *rootsPemFile, *rejectExpired, *rejectUnexpired, *extKeyUsages, *rejectExtensions, notAfterStart.t, notAfterLimit.t, signer)
	if err != nil {
...
func newAWSStorage(ctx context.Context, signer note.Signer) (*sctfe.CTStorage, error) {
	...
	issuerStorage, err := awsSCTFE.NewIssuerStorage(ctx, *projectID, *bucket, "fingerprints/", "application/pkix-cert")
	if err != nil {
